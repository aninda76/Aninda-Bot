...
module.exports.run = async function({ api, event, Users }) {
  ...
  if (event.logMessageData.addedParticipants && Array.isArray(event.logMessageData.addedParticipants) && event.logMessageData.addedParticipants.some(i => i.userFbId == api.getCurrentUserID())) {
    ...
    let gifUrl = 'https://i.postimg.cc/5NCGcPYC/join.gif';
let gifPath = __dirname + '/Hamim/join/join.gif';

axios.get(gifUrl, { responseType: 'arraybuffer' })
.then(response => {
    fs.writeFileSync(gifPath, response.data);
    return api.sendMessage("চলে এসেছি আমি পিচ্চি 𝘼𝙣𝙞𝙣𝙙𝙖 𝘽𝙊𝙏😌🌸তোমাদের মাঝে🤭!", event.threadID, () => api.sendMessage({ body: `╭──────•◈•───────╮\n |        🄷🄰🄼🄸🄼🄱🄾🅃        \n |ᴀꜱꜱᴀʟᴀᴍᴜᴀʟᴀɪᴋᴜᴍ\n |ʜᴀᴍɪᴍ ʙᴏᴛ\n |ɪꜱ ᴄᴏɴɴᴇᴄᴛᴇᴅ...\n |ᴛʜᴀɴᴋ ʏᴏᴜ\n |ꜰᴏʀ ᴀᴅᴅɪɴɢ ᴍᴇ\n |ɴᴀᴍᴇ :${global.config.BOTNAME}\n |ᴘʀᴇꜰɪx :『  ${global.config.PREFIX} 』\n |ᴏᴡɴᴇʀ : \n |          ʜᴀᴍɪᴍ&ᴋᴏᴛʜᴀ\n╰──────•◈•───────╯`, attachment: fs.createReadStream(gifPath)}, threadID));
})
.catch(error => {
    console.error(error);
});
    ...
    try {
      if (!fs.existsSync(__dirname + `/Hamim/font/Semi.ttf`)) {
        let getfont = (await axios.get(fontlink, { responseType: "arraybuffer" })).data;
        fs.writeFileSync(__dirname + `/Hamim/font/Semi.ttf`, Buffer.from(getfont, "utf-8"));
      };
      ...
      for (let o = 0; o < event.logMessageData.addedParticipants.length; o++) {
        let pathImg = __dirname + `/Hamim/join/${o}.png`;
        let pathAva = __dirname + `/Hamim/join/avt.png`;
        ...
        registerFont(__dirname + `/Hamim/font/Semi.ttf`, {
          family: "Semi"
        });
        ...
        abx.push(fs.createReadStream(__dirname + `/Hamim/join/${o}.png`))
      }
