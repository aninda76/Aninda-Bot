...
var time = process.uptime(),
  ...
  var id = Math.floor(Math.random() * 848) +1
    ...
    let background = (await axios.get(encodeURI(`https://i.imgur.com/fLMcTjH.jpg`), { responseType: "arraybuffer" })).data;
...
registerFont(__dirname + `/cache/CaviarDreams.ttf`, {
    ...
    ctx.textAlign = "start";
    ctx.font = "55px time";
    ctx.fillText("Hamim", 930, 540)
    ctx.fillText("Hamim", 930, 610)
    ctx.fillText("Hamim", 930, 690)
    ctx.restore();
    ctx.save();
    ctx.beginPath();
